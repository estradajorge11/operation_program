...
def main(): 
    st.title("Reserva Secundaria Down (MW)") 
    if 'mi_variable' not in st.session_state:
        st.session_state.mi_variable = ""
    
    if st.session_state.mi_variable != "":
        
        ruta = st.session_state.mi_variable + r"\Reserva Secundaria Down (MW).csv"
        df = pd.read_csv(ruta,index_col=r"ETAPA \ URS")      
        keys = df.keys()
        L = []
        df, keys, L = df_new(df,keys)
        figura(df,keys,1,L,"Reserva Secundaria Down (MW)",0.3,"MW","URS que no ofertan RSF Down")    
